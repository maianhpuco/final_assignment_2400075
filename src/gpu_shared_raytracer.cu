...
#define GPU_SHARED_INFINITY 1e20f
#define GPU_SHARED_PI 3.14159265358979323846f

__device__ float gpuSharedRandomFloat(curandState* state) {
    return curand_uniform(state);
}

__device__ bool gpuSharedHitSphere(const CudaSphere& sphere, const CudaRay& ray,
...
__device__ bool gpuSharedHitWorld(const CudaSphere* spheres, int numSpheres,
    ...
    for (int i = 0; i < numSpheres; i++) {
        if (gpuSharedHitSphere(spheres[i], ray, tMin, closest, tempPoint, tempNormal, tempMaterial, tempT)) {
            hitAnything = true;
            closest = tempT;
            hitPoint = tempPoint;
...
__device__ CudaVec3 gpuSharedSampleHemisphere(const CudaVec3& normal, curandState* state) {
    float x0 = gpuSharedRandomFloat(state);
    float x1 = gpuSharedRandomFloat(state);
    float phi = 2.0f * GPU_SHARED_PI * x1;
...
__device__ CudaVec3 gpuSharedRayColor(const CudaRay& ray, const CudaSphere* spheres,
    ...
    for (int bounce = 0; bounce < maxBounces; bounce++) {
        ...
        if (gpuSharedHitWorld(spheres, numSpheres, currentRay, 0.001f, GPU_SHARED_INFINITY,
            ...
            CudaVec3 newDir = gpuSharedSampleHemisphere(hitNormal, state);
...
__global__ void gpuSharedInitRandStatesKernel(curandState* states, int width, int height, unsigned long seed) {
...
// Path tracing kernel with SHARED MEMORY optimization

__global__ void gpuSharedPathTracingKernel(
    float* outputBuffer,
    float* accumBuffer,
    const CudaSphere* globalSpheres,  // Spheres in global memory
    int numSpheres,
    ...
    float offsetX = gpuSharedRandomFloat(localState) - 0.5f;
    float offsetY = gpuSharedRandomFloat(localState) - 0.5f;
    ...
    // =========================================================================
    // Trace ray using SHARED MEMORY spheres (fast access)
    // =========================================================================
    CudaVec3 color = gpuSharedRayColor(ray, sharedSpheres, spheresToLoad, maxBounces, localState);
...
void initGpuSharedRandStates(void* states, int width, int height, unsigned long seed) {
    ...
    gpuSharedInitRandStatesKernel<<<gridSize, blockSize>>>((curandState*)states, width, height, seed);
    CHECK_CUDA_SHARED(cudaGetLastError());
    CHECK_CUDA_SHARED(cudaDeviceSynchronize());
}
...
GpuSharedRaytracer::GpuSharedRaytracer()
    : d_spheres(nullptr)
    ...
    int imageHeight)
{
    ...
    CHECK_CUDA_SHARED(cudaMalloc(&d_spheres, numSpheres * sizeof(CudaSphere)));
    CHECK_CUDA_SHARED(cudaMemcpy(d_spheres, spheres, numSpheres * sizeof(CudaSphere), cudaMemcpyHostToDevice));
    
    CHECK_CUDA_SHARED(cudaMalloc(&d_accumBuffer, numPixels * 4 * sizeof(float)));
    CHECK_CUDA_SHARED(cudaMemset(d_accumBuffer, 0, numPixels * 4 * sizeof(float)));
    
    CHECK_CUDA_SHARED(cudaMalloc(&d_outputBuffer, numPixels * 4 * sizeof(float)));
    
    CHECK_CUDA_SHARED(cudaMalloc(&d_randStates, numPixels * sizeof(curandState)));
    
    initGpuSharedRandStates(d_randStates, imageWidth, imageHeight, 5678);
    
    m_initialized = true;
    
    printf("GPU Ray Tracer (Shared Memory) initialized:\n");
    printf("  Image: %d x %d\n", imageWidth, imageHeight);
    printf("  Spheres: %d (max shared: %d)\n", numSpheres, MAX_SHARED_SPHERES);
    printf("  Shared memory per block: %zu bytes\n", 
           (size_t)((numSpheres < MAX_SHARED_SPHERES ? numSpheres : MAX_SHARED_SPHERES) * sizeof(CudaSphere)));
}

void GpuSharedRaytracer::setNumBounces(int numBounces)
{
    m_numBounces = (numBounces > 0) ? numBounces : 1;
}

void GpuSharedRaytracer::setAccumulation(bool enable)
{
...
void GpuSharedRaytracer::resetAccumulation()
{
    m_sampleCount = 0;
    if (d_accumBuffer && m_initialized)
    {
        CHECK_CUDA_SHARED(cudaMemset(d_accumBuffer, 0, m_imageWidth * m_imageHeight * 4 * sizeof(float)));
    }
}

void GpuSharedRaytracer::render(float* outputImage)
{
    ...
    launchGpuSharedPathTracingKernel(
        d_outputBuffer,
        d_accumBuffer,
        d_spheres,
        m_numSpheres,
    ...
    cudaEventRecord(start);
    CHECK_CUDA_SHARED(cudaMemcpy(outputImage, d_outputBuffer,
...
GpuRenderStats GpuSharedRaytracer::getStats() const
{
...
void GpuSharedRaytracer::cleanup()
{
